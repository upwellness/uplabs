...
|---|---|---|
| 2026-09-14 | **Body Scan (`/v2/bca` ดูผลแบบสแกน): จิ้มจุดบนร่างกาย/ชิปค่า → popup แปลผลด้วยภาพ** ไม่เลื่อนหน้า — iframe `postMessage({type:"uplabs-bca-zone",key})` → `BcaScanReveal` เปิด `components/bca/MetricPopup.tsx` (ตรวจ `e.source` = iframe) · เพิ่มโซนแตะ **อายุร่างกาย** (หัว) และ **BMI** (ลำตัวบน) · popup 6 ค่า: %ไขมัน ภาพแยกเพศ · กล้ามเนื้อ ไม้บรรทัดแยกเพศ · ช่องท้อง โมเดล+แกลเลอรี · อายุร่างกาย สเกลเทียบอายุจริง · BMI ไม้บรรทัด · BMR · Esc ปิด popup ก่อน แล้วค่อยปิดสแกน | __HASH__ |
| 2026-09-14 | `/v2/bca` (โค้ช) เพิ่มการ์ด **"แปลผลด้วยภาพ"** ใต้ผลล่าสุด — ภาพ % ไขมัน / ไม้บรรทัดกล้ามเนื้อแยกเพศ / โมเดลไขมันช่องท้อง+แกลเลอรี ชุดเดียวกับหน้าลูกค้า · ย้าย art ไป `components/bca/BcaArt.tsx` ใช้ร่วม 2 หน้า | `d94aafe` |
| 2026-09-14 | BCA Reveal รอบ 3 (ต้นสั่ง): % กล้ามเนื้อเป็นไม้บรรทัด 4 ช่วง**แยกเพศ**ชี้ค่าลูกค้า · ไขมันช่องท้องเพิ่มแกลเลอรี 3 ภาพตัวอย่างแตะขยายเต็มจอ + เรียกเป็น "คะแนน" (ไม่ใช่ %) ทุกจุด · L2 ระบุ **Nutrilite** ที่ให้สารอาหารนั้น (Cal Mag D / Triple Omega / All Plant Protein) ยังไม่มีปริมาณ · test ห้าม dose | `936041c` |
| 2026-09-14 | BCA Reveal: การ์ด % ไขมัน + ไขมันช่องท้อง เปลี่ยนจาก SVG เป็น**ภาพจริง**พร้อมตำแหน่งลูกค้า — fat% แยกเพศ 6 ภาพ (ชาย 3–40% · หญิง 10–40%) ลูกศร+เส้นชี้ค่าจริง interpolate ระหว่างจุดที่วัดจากภาพ · visceral โมเดลช่องท้อง ระดับ 1→11 map ซ้าย→ขวา dim รอบ ๆ เหลือแถบสว่างที่ตำแหน่งลูกค้า · เกินขอบภาพบอกตรง ๆ · `app/r/bca/[token]/_art.tsx` + `public/bca/*.webp` (≤80 KB) | `4022d4f` |

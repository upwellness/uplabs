...
| `abo` | นักธุรกิจ/โค้ช UP Wellness | ลูกค้าของตัวเอง + downline (read-only) + ทุกแอปที่ได้รับสิทธิ์ |
| *(public link)* | ลูกค้า / ผู้สนใจ | เฉพาะหน้า token-gated ที่ถูกส่งลิงก์ให้ (ไม่ต้องล็อกอิน) |

- **Downline (MLM):** `profiles.parent_id` → ผู้ถูกเชิญกลายเป็น downline ของผู้เชิญอัตโนมัติ · โค้ชเห็นลูกค้าของ downline แบบอ่านอย่างเดียว

...
| **ผลตรวจ BCA ส่งลูกค้า** | `measurements.share_token` · `share_opened_at` | `/r/bca/<token>` public ต่อ 1 ครั้งที่ชั่ง · โค้ชออกจาก `/v2/bca` · `lib/bca-reveal/{engine,store}.ts` · ภาพ/สเกล `components/bca/BcaArt.tsx` (ใช้ร่วมกับ `/v2/bca` การ์ด "แปลผลด้วยภาพ") · เกณฑ์จาก `lib/medical-status.ts` เท่านั้น |
| **portal ลูกค้า** | `customers.portal_token` · `portal_first_opened_at` · **`portal_events`** (open · metric · explain · food_log · cgm_upload · `meta` jsonb) | `/my/<token>` token-gated ไม่มีบัญชี · หมุนได้ · **เวอร์ชันแอปมือถือ 14 ก.ย. 2026** (`lib/health-design/portal-data.ts` โหลดครั้งเดียว → `app/my/[token]/_m/*` client) · routes `/api/my/<token>/{food,cgm,explain,event}` · `portal_events` = log การใช้ (R7) + ตัวนับโควตา AI อธิบาย |
...
| Cron | `/api/line/push-tomorrow` 18:00 · `/api/cron/nightly` 03:00 (Google Health sync → LINE nudge จันทร์ → snapshot) — Hobby plan จำกัด 2 cron |

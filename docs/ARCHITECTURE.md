...
| **ผลตรวจ BCA ส่งลูกค้า** | `measurements.share_token` · `share_opened_at` | `/r/bca/<token>` public ต่อ 1 ครั้งที่ชั่ง · โค้ชออกจาก `/v2/bca` · `lib/bca-reveal/{engine,store}.ts` · เกณฑ์จาก `lib/medical-status.ts` เท่านั้น |

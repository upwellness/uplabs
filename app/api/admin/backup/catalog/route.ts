...
import { orderTables, FOREIGN_TABLES } from "@/lib/backup/snapshot";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ catalog, order: orderTables(catalog), foreign: FOREIGN_TABLES, totals: { tables: catalog.length, rows: catalog.reduce((a, c) => a + (c.exact_rows ?? 0), 0) } }, { headers: { "cache-control": "no-store" } });

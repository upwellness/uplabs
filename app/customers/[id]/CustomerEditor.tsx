...
import { DOBPicker } from "../_components/DOBPicker";
import { computeAge, formatBothYears } from "@/lib/thai-date";
...
export function CustomerEditor({ customer }: { customer: Customer }) {
  ...
  const [birthDate, setBirthDate] = useState<string | null>(customer.birth_date);
  ...
  const age = computeAge(customer.birth_date)
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(`/api/customers/${customer.id}`, {
        ...
        body: JSON.stringify({ name, gender, birth_date: birthDate, height }),
      });
  ...
  const dobDisplay = customer.birth_date
    ? formatBothYears(customer.birth_date)
    : customer.birth_year
    ? `เกิด ${customer.birth_year + 543} (${customer.birth_year})`
    : null;
    ...
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="flex items-start gap-5">
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 font-thai text-sm text-ink-60">
            <span>{customer.gender === "male" ? "ชาย" : customer.gender === "female" ? "หญิง" : "—"}</span>
            {age != null && <><Dot /><span>{age} ปี</span></>}
            {dobDisplay && <><Dot /><span>{dobDisplay}</span></>}
